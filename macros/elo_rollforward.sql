...
{% set temp_ratings %}
    CREATE OR REPLACE TABLE workings_ratings AS (
        SELECT team, "ELO rating"::real as elo_rating, "ELO rating"::real AS original_rating
        FROM raw_team_ratings
        GROUP BY ALL
    )
{% endset %}
